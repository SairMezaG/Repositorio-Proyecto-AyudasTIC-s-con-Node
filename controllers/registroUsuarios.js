const { registroUsuariosModel, storageModel } = require("../models/index.js");
...
const getregistroUsuarios = async (req, res) => {
    try {
        const data = await registroUsuariosModel.find({}).populate('foto'); 
        res.send({ data });
    } catch (error) {
...
const getregistroUsuariosId = async (req, res) => {
    try {
        const { id } = req.params;  
        const data = await registroUsuariosModel.findById(id).populate('foto'); 
        if (!data) {
...
const postregistroUsuarios = async (req, res) => {
    try {
        ...
        const dataUser = await registroUsuariosModel.create(userData);
...
const updateregistroUsuarios = async (req, res) => {
    ...
    try {
        ...
        const data = await registroUsuariosModel.findOneAndUpdate({ _id: userId }, updatedData, { new: true });
        if (!data) {
            res.send({ message: "Usuario no existe", data });
        }
...
const deleteregistroUsuarios = async (req, res) => {
    ...
    try {
        await registroUsuariosModel.findOneAndDelete({ _id: userId });
...
module.exports = { getregistroUsuarios, postregistroUsuarios, updateregistroUsuarios, deleteregistroUsuarios, getregistroUsuariosId };
